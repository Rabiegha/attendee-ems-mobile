...
const mapEventFromBackend = (backendEvent: any): Event => {
  ...
  if (
    backendEvent.location_type === "physical" ||
    backendEvent.location_type === "hybrid"
  ) {
  ...
  return {
    ...
    location: location,
    status:
      backendEvent.status === "published" ? "upcoming" : backendEvent.status,
...
export const eventsService = {
  ...
  async getEventById(id: string): Promise<Event> {
    try {
      ...
      console.log(
        "[EventsService] Event fetched successfully:",
        response.data.name,
      );
  ...
  async getEventStats(id: string): Promise<EventStats> {
    try {
      ...
      console.log(
        "[EventsService] Event stats fetched successfully:",
        response.data,
      );
  ...
  getEventAttendeeTypes: async (eventId: string) => {
    try {
      console.log(
        "[EventsService] Fetching attendee types for event:",
        eventId,
      );
      const response = await axiosClient.get(
        `/events/${eventId}/attendee-types`,
      );
      console.log(
        "[EventsService] Attendee types fetched successfully:",
  ...
  getEventRegistrationFields: async (eventId: string) => {
    try {
      console.log(
        "[EventsService] Fetching registration fields for event:",
        eventId,
      );
      const response = await axiosClient.get(`/events/${eventId}`);
      const registrationFields =
        response.data.settings?.registration_fields || [];
      console.log(
        "[EventsService] Registration fields fetched:",
